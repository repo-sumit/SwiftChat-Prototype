...
export function aiPipelineLabel({ persona } = {}) {
  return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-family:${FONT};font-size:10px;font-weight:600;letter-spacing:0.4px;color:${C.textSecondary};text-transform:uppercase;margin-bottom:8px">
    <span>💬 Natural Language</span>
    <span style="color:${C.textTertiary}">›</span>
    <span style="color:${C.brand}">⚡ AI Analytics Engine</span>
    <span style="color:${C.textTertiary}">›</span>
    <span>📊 Results + 💡 Insight</span>
    ${persona ? `<span style="margin-left:auto;background:${C.brandSubtle};color:${C.brand};padding:2px 8px;border-radius:999px;font-size:10px;font-weight:600;letter-spacing:0.3px">${esc(persona)}</span>` : ''}
  </div>`
}
...
export function aiSqlBlock({ sql, mode = 'business' }) {
  if (mode === 'technical' && sql) {
    return `<div style="margin-top:8px;background:${C.aiCodeBg};border-radius:10px;overflow:hidden;font-family:'SFMono-Regular',Menlo,Consolas,monospace">
      ...
      <pre style="margin:0;padding:12px 14px;color:#E2EAFF;font-size:11px;line-height:18px;white-space:pre-wrap;word-break:break-word">${esc(sql)}</pre>
    </div>`
  }
  return `<div style="margin-top:8px;background:${C.brandSubtle};border:1px solid ${C.aiCardBorder};border-radius:10px;padding:10px 12px;font-family:${FONT}">
...
export function aiResultsTable({ rows, label = '📊 Results' }) {
  if (!rows || rows.length === 0) {
    return `<div style="margin-top:8px;padding:16px;border:1px solid ${C.borderDefault};border-radius:10px;background:${C.surface};color:${C.textTertiary};font-family:${FONT};font-size:13px;text-align:center">No data.</div>`
  }
  ...
  return `<div style="margin-top:8px;background:${C.surface};border:1px solid ${C.borderDefault};border-radius:10px;overflow:hidden;font-family:${FONT}">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:${C.surface};border-bottom:1px solid ${C.borderSubtle}">
      <span style="font-size:12px;font-weight:600;letter-spacing:0.2px;color:${C.textPrimary}">${esc(label)}</span>
      <span style="font-size:10px;color:${C.textTertiary};letter-spacing:0.3px">${rows.length} ${rows.length === 1 ? 'row' : 'rows'}</span>
    </div>
...
export function aiInsightCard(text) {
  if (!text) return ''
  return `<div style="margin-top:8px;background:${C.successBanner};border:1px solid ${C.success};border-radius:10px;padding:12px 14px;font-family:${FONT};display:flex;gap:10px;align-items:flex-start">
    ...
    <div style="flex:1;min-width:0">
      ...
      <div style="font-size:13px;line-height:18px;color:${C.successText};font-weight:500">${esc(text)}</div>
    </div>
  </div>`
}
...
export function aiDeepDiveProgress({ turnIndex, totalTurns, title }) {
  ...
  return `<div style="margin-bottom:8px;display:flex;align-items:center;gap:10px;font-family:${FONT};font-size:11px;color:${C.textSecondary};letter-spacing:0.2px">
    <span style="font-weight:700;color:${C.brand};text-transform:uppercase;font-size:10px">Deep Dive</span>
    <span style="color:${C.textTertiary}">›</span>
    <span style="font-weight:600;color:${C.textPrimary}">${esc(title)}</span>
    <span style="margin-left:auto;display:inline-flex;align-items:center;gap:6px">${dots}<span style="font-weight:600;color:${C.textPrimary};margin-left:4px">Q${turnIndex + 1}/${n}</span></span>
  </div>`
}
...
export function aiResultCardHtml({
  ...
  return `<div style="font-family:${FONT};color:${C.textPrimary}">
    ...
    <div style="font-size:14px;font-weight:600;line-height:18px;letter-spacing:-0.1px;color:${C.textPrimary};margin-bottom:4px">${esc(question)}</div>
    ${fallbackNotice ? `<div style="font-size:11px;color:${C.warningText};background:${C.warningBanner};border:1px solid ${C.warning};border-radius:8px;padding:8px 10px;margin-top:6px;margin-bottom:6px">${esc(fallbackNotice)}</div>` : ''}
